...
void cg_logwork(struct work *work, unsigned char *nonce_bin, bool ok)
{
	if(opt_logwork_path) {
		...
		sprintf(szmsg, "%s %08x midstate %s data %s nonce %s diff %I64d", ok?"o":"x", work->id, szmidstate, szdata, sznonce5, work->share_diff);
			...
			if(g_logwork_file) {
				...
				if(ok) {
					if(g_logwork_asicnum == 1) {
						sprintf(szmsg, "midstate %s data %s nonce %s", szmidstate, szdata, sznonce4);
					...
					if(opt_logwork_diff) {
						int diffnum = 0;
						uint64_t difftmp = work->share_diff;
						while(1) {
							difftmp = difftmp >> 1;
							if(difftmp > 0) {
								diffnum++;
						...
						applog(LOG_ERR, "work diff %I64d diffnum %d", work->share_diff, diffnum);
						sprintf(szmsg, "midstate %s data %s nonce %s", szmidstate, szdata, sznonce4);
